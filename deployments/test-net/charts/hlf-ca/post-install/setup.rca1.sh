...
kubectl -n n1 exec $POD_RCA -- fabric-ca-client enroll -u http://rca1-admin:rca1-adminPW@0.0.0.0:7054
printMessage "enroll rca1-admin" $?
...
kubectl -n n1 exec $POD_RCA -- sh -c "fabric-ca-client register --id.name admin-peer0.org1.net --id.secret admin-peer0.org1.netPW --id.type admin --id.attrs \"hf.Registrar.Attributes=*,hf.Revoker=true,hf.GenCRL=true,admin=true:ecert\" -u http://0.0.0.0:7054 > /dev/null"
printMessage "register orgadmin admin-peer0.org1.net" $?

kubectl -n n1 exec $POD_RCA -- sh -c "FABRIC_CA_CLIENT_HOME=/var/hyperledger/crypto-config/Org1MSP/admin fabric-ca-client enroll -u http://admin-peer0.org1.net:admin-peer0.org1.netPW@0.0.0.0:7054"
printMessage "enroll orgadmin admin-peer0.org1.net" $?
######## 5. Register peer(s) for Org1MSP
kubectl -n n1 exec $POD_RCA -- sh -c "fabric-ca-client register --id.name peer0.org1.net --id.secret peer0.org1.netPW --id.type peer -u http://0.0.0.0:7054 > /dev/null"
printMessage "register peer peer0.org1.net" $?
######## 6. Enroll peer(s) for Org1MSP
kubectl -n n1 exec $POD_RCA -- sh -c "FABRIC_CA_CLIENT_HOME=/var/hyperledger/crypto-config/Org1MSP/peer0.org1.net fabric-ca-client enroll -u http://peer0.org1.net:peer0.org1.netPW@0.0.0.0:7054"
