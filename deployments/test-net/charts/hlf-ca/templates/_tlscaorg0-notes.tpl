...
kubectl -n {{ $.Release.Namespace }} exec $POD_TLS_CA -- fabric-ca-client register -d --id.name {{ . }} --id.secret orderer0.org0.comPW --id.type orderer -u http://0.0.0.0:7054
{{- end }}
...
kubectl -n {{ $.Release.Namespace }} exec $POD_TLS_CA -- cp ./Org0MSP/tls/admin/msp/cacerts/0-0-0-0-7054.pem ./Org0MSP/{{ . }}/tls-ca-cert.pem
