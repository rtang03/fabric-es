...
kubectl -n {{ .Release.Namespace }} exec $POD_TLSCA0 -- sh -c "fabric-ca-client enroll -d -u http://{{ .Values.caAdmin }}:{{ .Values.caAdminPW }}@0.0.0.0:7054"
...
kubectl -n {{ $.Release.Namespace }} exec $POD_TLSCA0 -- sh -c "fabric-ca-client register -d --id.name {{ .id }} --id.secret {{ .pass }} --id.type orderer -u http://0.0.0.0:7054"
{{- end }}
