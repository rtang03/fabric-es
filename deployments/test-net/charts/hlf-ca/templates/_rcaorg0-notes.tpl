...
kubectl -n {{ $.Release.Namespace }} exec $POD_RCA -- sh -c "fabric-ca-client register -d --id.name {{ .id }} --id.secret {{ .pass }} --id.type orderer -u http://0.0.0.0:7054"
{{- end }}
...
sleep 1
{{- range .Values.orderers }}
kubectl -n {{ $.Release.Namespace }} exec $POD_RCA -- sh -c "cp ./{{ $.Values.mspId }}/ca/admin/msp/cacerts/0-0-0-0-7054.pem ./{{ $.Values.mspId }}/{{ .id }}/{{ $.Values.ordererOrg.domain }}-ca-cert.pem"
{{- end }}
sleep 1
...
sleep 1
{{- range .Values.orderers }}
kubectl -n {{ $.Release.Namespace }} exec $POD_RCA -- sh -c "cp ./{{ $.Values.mspId }}/admin/msp/signcerts/cert.pem ./{{ $.Values.mspId }}/{{ .id }}/msp/admincerts/{{ $.Values.ordererOrg.domain }}-admin-cert.pem"
{{- end }}
...
kubectl -n {{ .Release.Namespace }} exec $POD_RCA -- sh -c "cp ./{{ $.Values.mspId }}/{{ .Values.ordererOrg.firstOrderer }}/msp/admincerts/{{ .Values.ordererOrg.domain }}-admin-cert.pem ./{{ $.Values.mspId }}/msp/admincerts"
