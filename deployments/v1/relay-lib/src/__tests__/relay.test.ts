...
afterAll(async () => {
  ...
  return new Promise<void>((ok) => setTimeout(() => ok(), 1000));
});
...
describe('Process Message', () => {
  it('should reject undefined or null message', () => {
    expect(
      processMessage({ message: undefined, client: publisher, topic: 'rejection' })
    ).rejects.toThrow();
  });
  ...
  it('should publish with no subscriber', async () => {
    await processMessage({ message: msg, client: publisher, topic: '0 subscriber' }).then(
      (numOfSub) => {
  ...
  it('should publish with 2 subscribers', async () => {
    ...
    await processMessage({ message: msg, client: publisher, topic }).then((numOfSub) => {
      expect(numOfSub).toEqual(0);
    });
  });
...
describe('Relay Service', () => {
  it('should reject undefined or null url', () => {
    const f = () => {
      relayService({ targetUrl: undefined, client: publisher, topic: 'rejection', isHttps: false });
    };
    expect(f).toThrow();
  });
  ...
  it('should reject undefined or null client', () => {
    const f = () => {
      relayService({ targetUrl, client: null, topic: 'rejection', isHttps: false });
    };
    expect(f).toThrow();
  });
  ...
  it('should reject undefined, null or empty string topic', () => {
    const f = () => {
      relayService({ targetUrl, client: publisher, topic: null, isHttps: false });
    };
    expect(f).toThrow();
  });
