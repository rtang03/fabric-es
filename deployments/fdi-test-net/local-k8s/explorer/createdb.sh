...
${PSQL} -v dbname=$DATABASE -v user=$USER -v passwd=$PASSWD -f /var/artifacts/explorer/db/explorerpg.sql ;
${PSQL} -v dbname=$DATABASE -v user=$USER -v passwd=$PASSWD -f /var/artifacts/explorer/db/updatepg.sql ;;
esac
