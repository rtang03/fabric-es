...
//   enableGCPLogger?: boolean;
// }) => Logger = ({
//   name,
//   sendToConsole = true,
//   enableGCPLogger = process.env.NODE_ENV
// }) => {
...
export const getLogger = (name: string, sendToConsole?: boolean, enableGCPLogger?: boolean) => {
  let transportArray: any[] = [
    new transports.File({ filename: `./logs/all.log` }),
    new transports.File({
      filename: `./logs/error.log`,
  ...
  if (enableGCPLogger) transportArray = [new LoggingWinston({ projectId: 'fdi' }), ...transportArray];
