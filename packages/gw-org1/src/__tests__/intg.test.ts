...
import {
  APPLY_LOAN, APPROVE_LOAN, CANCEL_LOAN, CREATE_DOCUMENT, DELETE_DOCUMENT,
  ...
  REJECT_LOAN, RESTRICT_DOCUMENT_ACCESS, RETURN_LOAN, UPDATE_DOCUMENT, UPDATE_LOAN
} from '@espresso/model-loan';
import {
  CREATE_DATA_DOC_CONTENTS, CREATE_FILE_DOC_CONTENTS, CREATE_LOAN_DETAILS,
...
import {
  GET_COMMITS_BY_DOCUMENT, GET_COMMITS_BY_LOAN, GET_DOCUMENT_BY_ID, GET_LOAN_BY_ID,
  GW_REGISTER_ENROLL,
...
describe('Unit Test: Org1 Apply Loans', () => {
      ...
      await request(gateway)
        ...
        .send({
          ...
          variables: {
            userId: 'unitTestUser',
            loanId: loanId0,
            registration: `BR-UNIT-TEST`, companyName: 'Unit Test and Co. Ltd.',
  ...
  it('apply loan 1', async () => {
    if (isReady) {
      await request(gateway).post('/graphql').set('authorization', `bearer ${accessToken}`)
        .send({ operationName: 'ApplyLoan', query: APPLY_LOAN.loc.source.body, variables: {
          userId: 'unitTestUser', loanId: loanId1, description: 'Org1 unit test loan 1', reference: 'REF-UNIT-TEST-LOAN-1'
        }}).expect(({ body: { data, errors }}) => expect(data.applyLoan.id).toEqual(loanId1)).catch(_ => expect(false).toBeTruthy());
      return;
    }
    expect(false).toBeTruthy();
  });
  ...
  it('apply loan 2', async () => {
    if (isReady) {
      await request(gateway).post('/graphql').set('authorization', `bearer ${accessToken}`)
        .send({ operationName: 'ApplyLoan', query: APPLY_LOAN.loc.source.body, variables: {
          userId: 'unitTestUser', loanId: loanId2, description: 'Org1 unit test loan 2', reference: 'REF-UNIT-TEST-LOAN-2'
        }}).expect(({ body: { data, errors }}) => expect(data.applyLoan.id).toEqual(loanId2)).catch(_ => expect(false).toBeTruthy());
      return;
    }
    expect(false).toBeTruthy();
  });
  ...
  it('apply loan 3', async () => {
    if (isReady) {
      await request(gateway).post('/graphql').set('authorization', `bearer ${accessToken}`)
        .send({ operationName: 'ApplyLoan', query: APPLY_LOAN.loc.source.body, variables: {
          userId: 'unitTestUser', loanId: loanId3, description: 'Org1 unit test loan 3', reference: 'REF-UNIT-TEST-LOAN-3'
        }}).expect(({ body: { data, errors }}) => expect(data.applyLoan.id).toEqual(loanId3)).catch(_ => expect(false).toBeTruthy());
      return;
    }
    expect(false).toBeTruthy();
  });
  ...
  it('apply loan 4', async () => {
    if (isReady) {
      await request(gateway).post('/graphql').set('authorization', `bearer ${accessToken}`)
        .send({ operationName: 'ApplyLoan', query: APPLY_LOAN.loc.source.body, variables: {
          userId: 'unitTestUser', loanId: loanId4, description: 'Org1 unit test loan 4', reference: 'REF-UNIT-TEST-LOAN-4'
        }}).expect(({ body: { data, errors }}) => expect(data.applyLoan.id).toEqual(loanId4)).catch(_ => expect(false).toBeTruthy());
      return;
    }
    expect(false).toBeTruthy();
  });
  ...
  it('apply loan 5', async () => {
    if (isReady) {
      await request(gateway).post('/graphql').set('authorization', `bearer ${accessToken}`)
        .send({ operationName: 'ApplyLoan', query: APPLY_LOAN.loc.source.body, variables: {
          userId: 'unitTestUser', loanId: loanId5, description: 'Org1 unit test loan 5', reference: 'REF-UNIT-TEST-LOAN-5'
        }}).expect(({ body: { data, errors }}) => expect(data.applyLoan.id).toEqual(loanId5)).catch(_ => expect(false).toBeTruthy());
      return;
    }
    expect(false).toBeTruthy();
  });
...
describe('Unit Test: Org1 Create Documents', () => {
  ...
  it('create document 3', async () => {
    if (isReady) {
      await request(gateway).post('/graphql').set('authorization', `bearer ${accessToken}`)
        .send({ operationName: 'CreateDocument', query: CREATE_DOCUMENT.loc.source.body, variables: {
            userId: 'unitTestUser', documentId: documentId3, loanId: loanId1, title: 'Org1 unit test document 3', reference: 'REF-UNIT-TEST-DOC-3'
        }}).expect(({ body: { data, errors }}) => expect(data.createDocument.id).toEqual(documentId3)).catch(_ => expect(false).toBeTruthy());
      return;
    }
    expect(false).toBeTruthy();
  });
  ...
  it('create document 4', async () => {
    if (isReady) {
      await request(gateway).post('/graphql').set('authorization', `bearer ${accessToken}`)
        .send({ operationName: 'CreateDocument', query: CREATE_DOCUMENT.loc.source.body, variables: {
            userId: 'unitTestUser', documentId: documentId4, loanId: loanId2, title: 'Org1 unit test document 4', reference: 'REF-UNIT-TEST-DOC-4'
        }}).expect(({ body: { data, errors }}) => expect(data.createDocument.id).toEqual(documentId4)).catch(_ => expect(false).toBeTruthy());
      return;
    }
    expect(false).toBeTruthy();
  });
  ...
  it('create document 5', async () => {
    if (isReady) {
      await request(gateway).post('/graphql').set('authorization', `bearer ${accessToken}`)
        .send({ operationName: 'CreateDocument', query: CREATE_DOCUMENT.loc.source.body, variables: {
            userId: 'unitTestUser', documentId: documentId5, loanId: loanId3, title: 'Org1 unit test document 5', reference: 'REF-UNIT-TEST-DOC-5'
        }}).expect(({ body: { data, errors }}) => expect(data.createDocument.id).toEqual(documentId5)).catch(_ => expect(false).toBeTruthy());
      return;
    }
    expect(false).toBeTruthy();
  });
  ...
  it('create document 6', async () => {
    if (isReady) {
      await request(gateway).post('/graphql').set('authorization', `bearer ${accessToken}`)
        .send({ operationName: 'CreateDocument', query: CREATE_DOCUMENT.loc.source.body, variables: {
            userId: 'unitTestUser', documentId: documentId6, loanId: loanId4, title: 'Org1 unit test document 6', reference: 'REF-UNIT-TEST-DOC-6'
        }}).expect(({ body: { data, errors }}) => expect(data.createDocument.id).toEqual(documentId6)).catch(_ => expect(false).toBeTruthy());
      return;
    }
    expect(false).toBeTruthy();
  });
  ...
  it('create document 7', async () => {
    if (isReady) {
      await request(gateway).post('/graphql').set('authorization', `bearer ${accessToken}`)
        .send({ operationName: 'CreateDocument', query: CREATE_DOCUMENT.loc.source.body, variables: {
            userId: 'unitTestUser', documentId: documentId7, loanId: loanId5, title: 'Org1 unit test document 7', reference: 'REF-UNIT-TEST-DOC-7'
        }}).expect(({ body: { data, errors }}) => expect(data.createDocument.id).toEqual(documentId7)).catch(_ => expect(false).toBeTruthy());
      return;
    }
    expect(false).toBeTruthy();
  });
...
describe('Unit Test: Org1 Loans', () => {
  it('cancel loan', async () => {
    if (isReady) {
      await request(gateway).post('/graphql').set('authorization', `bearer ${accessToken}`)
        .send({ operationName: 'CancelLoan', query: CANCEL_LOAN.loc.source.body, variables: {
          userId: 'unitTestUser', loanId: loanId1
        }}).expect(({ body: { data, errors }}) => expect(data.cancelLoan.id).toEqual(loanId1)).catch(_ => expect(false).toBeTruthy());
      return;
    }
    expect(false).toBeTruthy();
  });
  ...
  it('approve loan', async () => {
    if (isReady) {
      await request(gateway).post('/graphql').set('authorization', `bearer ${accessToken}`)
        .send({ operationName: 'ApproveLoan', query: APPROVE_LOAN.loc.source.body, variables: {
          userId: 'unitTestUser', loanId: loanId2
        }}).expect(({ body: { data, errors }}) => expect(data.approveLoan.id).toEqual(loanId2)).catch(_ => expect(false).toBeTruthy());
      return;
    }
    expect(false).toBeTruthy();
  });
  ...
  it('return loan', async () => {
    if (isReady) {
      await request(gateway).post('/graphql').set('authorization', `bearer ${accessToken}`)
        .send({ operationName: 'ReturnLoan', query: RETURN_LOAN.loc.source.body, variables: {
          userId: 'unitTestUser', loanId: loanId3
        }}).expect(({ body: { data, errors }}) => expect(data.returnLoan.id).toEqual(loanId3)).catch(_ => expect(false).toBeTruthy());
      return;
    }
    expect(false).toBeTruthy();
  });
  ...
  it('reject loan', async () => {
    if (isReady) {
      await request(gateway).post('/graphql').set('authorization', `bearer ${accessToken}`)
        .send({ operationName: 'RejectLoan', query: REJECT_LOAN.loc.source.body, variables: {
          userId: 'unitTestUser', loanId: loanId4
        }}).expect(({ body: { data, errors }}) => expect(data.rejectLoan.id).toEqual(loanId4)).catch(_ => expect(false).toBeTruthy());
      return;
    }
    expect(false).toBeTruthy();
  });
  ...
  it('reject loan', async () => {
    if (isReady) {
      await request(gateway).post('/graphql').set('authorization', `bearer ${accessToken}`)
        .send({ operationName: 'ExpireLoan', query: EXPIRE_LOAN.loc.source.body, variables: {
          userId: 'unitTestUser', loanId: loanId5
        }}).expect(({ body: { data, errors }}) => expect(data.expireLoan.id).toEqual(loanId5)).catch(_ => expect(false).toBeTruthy());
      return;
    }
    expect(false).toBeTruthy();
  });
  ...
  it('update loan 2', async () => {
    if (isReady) {
      await request(gateway).post('/graphql').set('authorization', `bearer ${accessToken}`)
        .send({ operationName: 'UpdateLoan', query: UPDATE_LOAN.loc.source.body, variables: {
          userId: 'unitTestUser', loanId: loanId2, description: 'Org1 unit test loan 2 EDITED'
        }}).expect(({ body: { data, errors }}) => expect(data.updateLoan[0].id).toEqual(loanId2)).catch(_ => expect(false).toBeTruthy());
      return;
    }
    expect(false).toBeTruthy();
  });
  ...
  it('update readonly field of loan 2', async () => {
    if (isReady) {
      await request(gateway).post('/graphql').set('authorization', `bearer ${accessToken}`)
        .send({ operationName: 'UpdateLoan', query: UPDATE_LOAN.loc.source.body, variables: {
          userId: 'unitTestUser', loanId: loanId2, reference: 'REF-UNIT-TEST-LOAN-2NEW'
...
describe('Unit Test: Org1 Queries', () => {
  ...
  it('query commits of loan', async () => {
    if (isReady) {
      await request(gateway).post('/graphql').send({
          operationName: 'GetCommitsByLoanId', query: GET_COMMITS_BY_LOAN, variables: { loanId: loanId3 }
        }).expect(({ body: { data, errors }}) => expect(data.getCommitsByLoanId).toMatchSnapshot()).catch(_ => expect(false).toBeTruthy());
      return;
    }
    expect(false).toBeTruthy();
  });
  ...
  it('query loan by id 1', async () => {
    if (isReady) {
      await request(gateway).post('/graphql').send({
        ...
        }).expect(({ body: { data, errors }}) => expect(data.getLoanById).toMatchSnapshot()).catch(_ => expect(false).toBeTruthy());
      return;
    }
    expect(false).toBeTruthy();
  });
  ...
  it('query loan by id 3', async () => {
    if (isReady) {
      await request(gateway).post('/graphql').send({
          operationName: 'GetLoanById', query: GET_LOAN_BY_ID, variables: { loanId: loanId3 }
        }).expect(({ body: { data, errors }}) => expect(data.getLoanById).toMatchSnapshot()).catch(_ => expect(false).toBeTruthy());
      return;
    }
    expect(false).toBeTruthy();
  ...
  it('query loan by id 5', async () => {
    if (isReady) {
      await request(gateway).post('/graphql').send({
          operationName: 'GetLoanById', query: GET_LOAN_BY_ID, variables: { loanId: loanId5 }
        }).expect(({ body: { data, errors }}) => expect(data.getLoanById).toMatchSnapshot()).catch(_ => expect(false).toBeTruthy());
      return;
    }
    expect(false).toBeTruthy();
