...
import type { LoanContext, Loan, OutputLoan } from './types';
  ...
  Query: {
    ...
    getLoanById: catchResolverErrors(
      async (
        ...
        const { data, status, error } = await repo.fullTextSearchEntity<OutputLoan>({
          entityName: 'loan',
          query: `@id:${loanId}`,
          cursor: 0,
          pagesize: 1,
    ...
    getPaginatedLoans: catchResolverErrors(
      ...
      ): Promise<Paginated<OutputLoan>> => {
        const { data, error, status } = await repo.fullTextSearchEntity<OutputLoan>({
          entityName: 'loan',
          query: `*`,
          cursor,
          pagesize: pageSize,
