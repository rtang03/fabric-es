...
describe('Loan Unit Test - Resolver', () => {
  ...
  it('update description of loan 2', async () =>
    createTestClient(service)
      ...
      .then(({ data }) =>
        expect(data.updateLoan.map((d) => (d && d.id ? d.id : ''))).toContain('L0002')
      ));
  ...
  it('update comment of loan 3', async () =>
    createTestClient(service)
      ...
      .then(({ data }) =>
        expect(data.updateLoan.map((d) => (d && d.id ? d.id : ''))).toContain('L0003')
      ));
  ...
  it('update description and comment of loan 4', async () =>
    createTestClient(service)
      ...
      .then(({ data }) =>
        expect(data.updateLoan.map((d) => (d && d.id ? d.id : ''))).toContain('L0004')
      ));
