  ...
  return new Promise<{ read: number; count: number }>(async (resolve, reject) => {
    ...
    if (!client)
      ...
      try {
        ...
        const msgs = (await client.zrangebyscore(topic, '-inf', Date.now())).map(str => {
          if (callback) {
            try {
              ...
              if (isReqRes(msg))
                ...
                logger.warn(`Existing message of unknown type: '${str}'`);
                callback(topic, null, str);
              }
            } catch (error) {
              logger.warn(`Existing non-JSON message: '${str}'`);
              callback(topic, null, str);
            }
          } else
            logger.info(`Existing message from '${topic}': '${str}'`);
          return str;
        });
        read = msgs.length;
