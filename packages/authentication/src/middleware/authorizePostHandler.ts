...
export const authorizePostHandler = (oauth: OAuth2Server, options: any = {}) => async (
  req: Express.Request,
  ...
  const { client_id, redirect_uri, state, response_type, grant_type } = req.body;
  ...
  options.authenticateHandler = {
    handle: async () => (req?.app?.locals?.user_id ? await OUser.findOne({ id: req.app.locals.user_id }) : { id: null })
  };

  await oauth
    .authorize(new Request(req), new Response(res), options)
    .then((code: AuthorizationCode) => {
      logger.info(util.format('authorization code is obtained: %s', !req?.app?.locals?.user_id));

      res.locals.oauth = { code };
      res.redirect(`${code.redirectUri}?code=${code.authorizationCode}&state=${state}`);
    })
    .catch(error => {
      logger.warn(util.format('authorization fail: %s', error.message));

      res.status(http.BAD_REQUEST).send({ ok: false, authorization_code: false, message: error.message });
    });
  next();
};
