...
export const createQueryHandlerService: (option: {
  ...
  isAddUserRepo = false,
    ...
    const reducers: Record<string, Reducer> = { [Organization.entityName]: getReducer(orgReducer) };
    redisRepos = composeRedisRepos(publisher, redisRepos)(Organization, { fields: orgIndices });

    if (isAddUserRepo) {
      entityNames.push(User.entityName);
      reducers[User.entityName] = getReducer(userReducer);
