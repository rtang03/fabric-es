      ...
      callback: async (channel: string, message: ReqRes, messageStr?: string): Promise<void> => {
        if (message) {
          ...
          if (!result.errors) {
            ...
            logger.debug(`[PERFTEST]{"id":"${message.id}","url":"${message.url.url}","method":"${message.method}","entities":[${rest.commits.reduce((a,c) => {return `${(a) ? `${a},` : ''}"${c.entityId}"`;}, '')}],"writeFabFinish":${Date.now()}}`);
          } else {
