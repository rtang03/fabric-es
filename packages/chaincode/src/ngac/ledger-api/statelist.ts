...
const stateList: <T extends Attribute | Attribute[] | Resource | Policy = any>(
...
} = <T extends Attribute | Attribute[] | Resource | Policy = any>(namespace, { stub }) => ({
  ...
  addState: async (key, item) => {
    await stub.putState(stub.createCompositeKey(namespace, splitKey(key)), serialize(item));
    return item;
  },
  getState: async key => {
    const data = await stub.getState(stub.createCompositeKey(namespace, splitKey(key)));
