...
//     range:
//       Math.round(
//         Math.log10(parseInt(process.env.RUNS_NUM, 10) * parseInt(process.env.BATCH_NUM, 10))
//       ) + 1,
//     stamp: Date.now(),
...
//     data: any,
//     pTestConfig?: PerfTestConfig
//   ): Promise<{ poId: string; actionResponse: string }[]> => {
//     return new Promise<
//       {
//         poId: string;
...
//           } else {
//             resolve(
//               data.map((d) => ({ invoiceId: d.invoiceId, actionResponse: d.actionResponse }))
//             );
//           }
//         });
//       } catch (error) {
...
//           if (res.status !== 200) {
//             reject(
//               `ERROR! Update payment status of Invoice failed (${res.status}): ${res.statusText}`
//             );
//           } else {
...
//             } else if (
//               result.data &&
//               result.data.fullTextSearchEntity &&
//               result.data.fullTextSearchEntity.items
//             ) {
...
//           if (count < 0)
//             console.log(
//               `${tag} read entities ${query} retrying in ${config.READ_WAIT} ms (${count})`
//             );
