...
//         console.log(
//           `[Test run ${run}][#${variant}] ERROR! Get access token failed for user u${variant}`
//         );
//         reject(index);
//         return;
//       }
...
//     let write = 0;
//     console.log(
//       `[Test run ${run}][#${variant}] Starting (ts ${procStarts})${
//         !accessToken && useAuth ? `, access token: ${token.substr(0, 70)}...` : '...'
//       }`
//     );
//
//     // PO processing
...
//             console.log(
//               `[Test run ${run}][#${variant}] Create POs stopped. Reading created POs failed`
//             );
//             reject(index);
//             return;
//           }
...
//           write += Date.now() - writeStart;
//           const shouldContinue = await Promise.all(
//             editedPoIds.map(async (p) =>
//               PerfTest.readEntities(
//                 `[Test run ${run}][#${variant}] Edit POs`,
//                 token,
//                 p,
//                 (results: any[]) =>
...
//           if (!shouldContinue.reduce((a, c) => a && c, true)) {
//             console.log(
//               `[Test run ${run}][#${variant}] Edit POs stopped. Reading edited POs failed`
//             );
//             reject(index);
//             return;
//           }
...
//           write += Date.now() - writeStart;
//           const shouldContinue = await Promise.all(
//             cancelledPoIds.map(async (p) =>
//               PerfTest.readEntities(
//                 `[Test run ${run}][#${variant}] Cancel POs`,
//                 token,
//                 p,
//                 (results: any[]) =>
...
//           if (!shouldContinue.reduce((a, c) => a && c, true)) {
//             console.log(
//               `[Test run ${run}][#${variant}] Cancel POs stopped. Reading cancelled POs failed`
//             );
//             reject(index);
//             return;
//           }
...
//           write += Date.now() - writeStart;
//           const shouldContinue = await Promise.all(
//             processPoResult.map(async (p) =>
//               PerfTest.readEntities(
//                 `[Test run ${run}][#${variant}] Process POs`,
//                 token,
//                 p.poId,
//                 (results: any[]) =>
//                   results.reduce((accu, r) => {
//                     return accu && p.actionResponse === '1'
//                       ? r.value.indexOf(`"status":2`) >= 0
//                       : r.value.indexOf(`"status":3`) >= 0;
//                   }, true)
//               )
//                 .then((_) => true)
//                 .catch((error) =>
//                   console.log(`[Test run ${run}][#${variant}] Process POs ${error}`)
//                 )
//             )
//           );
//           if (!shouldContinue.reduce((a, c) => a && c, true)) {
//             console.log(
//               `[Test run ${run}][#${variant}] Process POs stopped. Reading processed POs failed`
//             );
//             reject(index);
//             return;
//           }
...
//           const shouldContinue = await Promise.all(
//             invIds.map(async (v) =>
//               PerfTest.readEntities(`[Test run ${run}][#${variant}] Create Invoices`, token, v)
//                 .then((_) => true)
//                 .catch((error) =>
//                   console.log(`[Test run ${run}][#${variant}] Create Invoices ${error}`)
//                 )
//             )
//           );
//           if (!shouldContinue.reduce((a, c) => a && c, true)) {
//             console.log(
//               `[Test run ${run}][#${variant}] Create Invoices stopped. Reading created invoices failed`
//             );
//             reject(index);
//             return;
//           }
...
//           write += Date.now() - writeStart;
//           const shouldContinue = await Promise.all(
//             editedInvIds.map(async (v) =>
//               PerfTest.readEntities(
//                 `[Test run ${run}][#${variant}] Edit Invoices`,
//                 token,
//                 v,
//                 (results: any[]) =>
//                   results.reduce((accu, r) => accu && r.value.indexOf(`"status":1`) >= 0, true)
...
//                 .then((_) => true)
//                 .catch((error) =>
//                   console.log(`[Test run ${run}][#${variant}] Edit Invoices ${error}`)
//                 )
//             )
//           );
//           if (!shouldContinue.reduce((a, c) => a && c, true)) {
//             console.log(
//               `[Test run ${run}][#${variant}] Edit Invoices stopped. Reading edited invoices failed`
//             );
//             reject(index);
//             return;
//           }
...
//           write += Date.now() - writeStart;
//           const shouldContinue = await Promise.all(
//             notifiedInvIds.map(async (v) =>
//               PerfTest.readEntities(
//                 `[Test run ${run}][#${variant}] Transfer Invoices`,
//                 token,
//                 v,
//                 (results: any[]) =>
//                   results.reduce(
//                     (accu, r) => accu && r.value.indexOf(`,InvoiceTransferred`) >= 0,
//                     true
//                   )
//               )
//                 .then((_) => true)
//                 .catch((error) =>
//                   console.log(`[Test run ${run}][#${variant}] Trasnfer Invoices ${error}`)
//                 )
//             )
//           );
//           if (!shouldContinue.reduce((a, c) => a && c, true)) {
//             console.log(
//               `[Test run ${run}][#${variant}] Transfer Invoices stopped. Reading transferred invoices failed`
//             );
//             reject(index);
//             return;
//           }
...
//           write += Date.now() - writeStart;
//           const shouldContinue = await Promise.all(
//             confirmInvResult.map(async (v) =>
//               PerfTest.readEntities(
//                 `[Test run ${run}][#${variant}] Confirm Invoices`,
//                 token,
//                 v.invoiceId,
//                 (results: any[]) =>
//                   results.reduce((accu, r) => {
//                     return accu && v.actionResponse === '1'
//                       ? r.value.indexOf(`"status":2`) >= 0
//                       : r.value.indexOf(`"status":3`) >= 0;
//                   }, true)
//               )
//                 .then((_) => true)
//                 .catch((error) =>
//                   console.log(`[Test run ${run}][#${variant}] Confirm Invoices ${error}`)
//                 )
//             )
//           );
//           if (!shouldContinue.reduce((a, c) => a && c, true)) {
//             console.log(
//               `[Test run ${run}][#${variant}] Confirm Invoices stopped. Reading confirmed invoices failed`
//             );
//             reject(index);
//             return;
//           }
...
//           write += Date.now() - writeStart;
//           const shouldContinue = await Promise.all(
//             invFinInvIds.map(async (v) =>
//               PerfTest.readEntities(
//                 `[Test run ${run}][#${variant}] Update payment status`,
//                 token,
//                 v,
//                 (results: any[]) =>
//                   results.reduce(
//                     (accu, r) => accu && r.value.indexOf(`,PaymentStatusUpdated`) >= 0,
//                     true
//                   )
//               )
//                 .then((_) => true)
//                 .catch((error) =>
//                   console.log(`[Test run ${run}][#${variant}] Update payment status ${error}`)
//                 )
//             )
//           );
//           if (!shouldContinue.reduce((a, c) => a && c, true)) {
//             console.log(
//               `[Test run ${run}][#${variant}] Update payment status stopped. Reading updated invoices failed`
//             );
//             reject(index);
//             return;
//           }
...
//     if (useAuth) {
//       console.log(
//         `[Test run ${run}][#${variant}] Finished (ts ${runFinish}). E: ${elapsed}ms/${Math.round(
//           totalElapsed / totalRuns
//         )}s; A: ${auth}ms/${Math.round(totalAuth / totalRuns)}s; P: ${proc}ms/${Math.round(
//           totalProc / totalRuns
//         )}s; W: ${write}ms/${Math.round(totalWrite / totalRuns)}ms`
//       );
//     } else {
//       console.log(
//         `[Test run ${run}][#${variant}] Finished (ts ${runFinish}). Elapsed time: ${proc}ms/${Math.round(
//           totalProc / totalRuns
//         )}s; W: ${write}ms/${Math.round(totalWrite / totalRuns)}ms`
//       );
//     }
...
//
//   console.log(
//     `Running ${RUNS} x ${BATCH} relay tests (${RUNS_WAIT}ms, ${READ_WAIT}ms x ${READ_RETRY})...`
//   );
//   totalElapsed = 0;
//   totalAuth = 0;
//   totalProc = 0;
...
//     if (lessAuth === 'yes') {
//       try {
//         token = await PerfTest.authenticate(
//           `u${stamp}${run}`,
//           `${stamp}${run}@fake.it`,
//           'p@ssw0rd'
//         );
...
//       }
//       console.log(
//         `[Test run ${run}][Ran for ${((Date.now() - stamp) / 1000).toFixed(
//           3
//         )}s] Starting, next batch in ${runsWait}ms, access token: ${token.substr(0, 70)}...`
//       );
...
//         `[Test run ${run}][Ran for ${((Date.now() - stamp) / 1000).toFixed(
//           3
//         )}s] Starting, next batch in ${runsWait}ms...`
//       );
//     }
//
//     Promise.all(
...
//         if (lessAuth === 'yes') {
//           console.log(
//             `[Test run ${run}][Elapsed time ${((Date.now() - authStarts) / 1000).toFixed(
//               3
//             )}s] Completed: ${values.length} (A: ${Math.round(totalAuth / (i + 1))}s)`
//           );
//         } else {
//           console.log(
//             `[Test run ${run}][Elapsed time ${((Date.now() - authStarts) / 1000).toFixed(
//               3
//             )}s] Completed: ${values.length}`
//           );
//         }
//       })
//       .catch((errors) => {
